...
要配置不同类型的密钥存储，请参阅关于 [如何配置密钥存储]({{<ref secret-stores-overview>}}) 并审阅 [支持的密钥存储]({{<ref supported-secret-stores >}}) 查看不同密钥存储解决方案所需的具体细节。
## 获取密钥
...
- [配置密钥存储]({{<ref secret-stores-overview>}})
- [支持的密钥]({{<ref secret-stores-overview>}})
