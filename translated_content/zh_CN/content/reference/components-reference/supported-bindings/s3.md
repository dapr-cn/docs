...
| decodeBase64   | N  | 输出   | 在保存到存储桶之前解码 base64 文件内容的配置。 (保存有二进制内容的文件时)。 `true` 是唯一允许的正值。 Other positive variations like `"True", "1"` are not acceptable. Defaults to `false`                                                                                                                                             | `true`, `false`                |
