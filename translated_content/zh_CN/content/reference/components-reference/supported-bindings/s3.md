...
To setup an AWS S3 binding create a component of type `bindings.aws.s3`. This binding works with other S3-compatible services, such as Minio. 请参阅[本指南]({{< ref "howto-bindings.md#1-create-a-binding" >}})，了解如何创建和应用绑定配置。
...
| disableSSL     | N  | 输出   | Allows to connect to non `https://` endpoints. Defaults to `false`                                                                                                                                                                                      | `true`, `false`                |
| insecureSSL    | N  | 输出   | When connecting to `https://` endpoints, accepts invalid or self-signed certificates. Defaults to `false`                                                                                                                                               | `true`, `false`                |
