...
weight: 3000
description: "Dapr 要求每个入站 API 请求都需要包含一个认证token，然后才能放行"
---

默认情况下，Dapr 依靠网络边界来限制对其公共 API 的访问。 如果你打算将 Dapr API 暴露在网络边界之外，或者如果您的 deployment 需要额外级别的安全性，那么请考虑开启 Dapr API 的token认证。 这将使得 Dapr 要求每个入站 gRPC 和 HTTP API 请求都需要包含认证token，然后请求才能放行。
...
为了配置 API 身份验证，需要先使用任意 JWT token兼容工具(如https://jwt.io/) 和 secret 来生成您的token。
...
如果需要更新已配置的token，只需将 `DAPR_API_TOKEN` 环境变量设置为新值，然后重新启动 `daprd`进程。
...
> 注意，上述 secret 需要你希望开启 Dapr token 认证的命名空间中创建
...
如果需要更新已配置的token，只需将环境变量 `DAPR_API_TOKEN`设置为新值，然后重新启动 `daprd`进程。

### Kubernetes

如果需要更新在 Kubernates 中已配置的token，更新先前在每个命名空间中创建的 secret 的token。 您可以使用 `kubectl patch` 命令执行此操作，但更简单的方法是，使用 manifest 更新每个命名空间中的这些对象:
...
在 Dapr 中配置token认证后，所有客户端调用 Dapr API 的都必须要把 JWT 令牌附加到每个请求：
...
在 Kubernetes中，推荐将您的 secret mount 到 pod 的环境变量，如以下面示例中所示，一个叫做 `dapr-api-token` 的 Kubernetes secret 用于保存 token。
...
- 了解 [如何通过 token 认证来自 Dapr 的请求]({{< ref app-api-token.md >}})
