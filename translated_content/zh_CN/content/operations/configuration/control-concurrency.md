...
*请注意，这个速率限制对每一个来自Dapr的事件都是有保证的，这意味着Pub/Sub事件、来自其他服务的直接调用、绑定事件等等。 Dapr can't enforce the concurrency policy on requests that are coming to your app externally.*
