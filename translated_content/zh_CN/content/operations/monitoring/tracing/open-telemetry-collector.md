...
当 OpenTelemetry 项目进入到 GA（General Availability, 正式发布的版本）阶段时，Dapr 将会以 OpenTelemetry 的格式规范导出应用的追踪信息。 同时，可以使用 Zipkin 要求的格式导出应用追踪信息。 与 [OpenTelemetry Collector](https://github.com/open-telemetry/opentelemetry-collector) 结合一起使用，你可以将应用的追踪信息发送到许多流行的分布式追踪后端程序中（例如 Azure AppInsights，AWS X-Ray ，StackDriver 等）
...
2. 您已经设置好了分布式追踪后端程序用以接受应用追踪信息

3. 点击 [这里](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/exporter) 和 [这里](https://github.com/open-telemetry/opentelemetry-collector/tree/main/exporter) 去查看你的分布式追踪程序是否支持 OpenTelemetry Collector。 在这些外链的页面上，找到您要使用的应用追踪信息导出工具，并阅读其文档以查找所需的参数。
...
部署并运行一些应用程序。 等待追踪信息推送到您的分布式跟踪后端中，并在那里查看它们。

## 相关链接
* 尝试访问[可观察性快速入门](https://github.com/dapr/quickstarts/tree/master/observability/README.md)
* 如何设置 [tracing 配置选项]({{< ref "configuration-overview.md#tracing" >}})
