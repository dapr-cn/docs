...
| [**密钥**]({{<ref "secrets-overview.md">}})                           | `/v1.0/secrets`                   | Dapr提供一个密钥构建块API ，并与公共云存储，本地存储和 Kubernetes 等密钥存储集成，以存储密钥。 服务可以调用密钥API来获取密钥，例如获取数据库的连接字符串。    |
| [**Configuration (配置)**]({{<ref "configuration-api-overview.md">}}) | `/v1.0-alpha1/configuration`      | Dapr提供了一个配置API ，使您能够从支持的配置存储检索和订阅的应用程序配置项。 这使应用程序能够设置特定的配置信息，例如在启动时或在存储进行配置更改时。              | 
