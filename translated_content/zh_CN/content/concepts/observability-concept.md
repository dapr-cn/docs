...
### 分布式跟踪
Dapr 可以 [配置发送跟踪数据]({{X11X}})，并且由于 Dapr 使用广泛采用的协议（如 [Zipkin](https://zipkin.io) 协议）进行跟踪，因此可以轻松地集成多个 [监控后端]({{X12X}})。
...
### 跟踪上下文
Dapr 使用 [W3C 跟踪]({{X20X}}) 规范来跟踪上下文，并可以生成和传播上下文头本身或传播用户提供的上下文头。
...
### 健康状态
Dapr sidecar 暴露了 [健康检查]({{X30X}})的 HTTP 终结点。 通过此终结点，可以探测 Dapr 进程或 sidecar，以确定它的准备度和活跃度，并采取相应的行动。 
