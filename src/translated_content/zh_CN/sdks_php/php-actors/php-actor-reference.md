...
Actor 方法调用在 `php-fpm` ， `nginx` 或 WINDOWS 平台上的 IIS 的生产配置是非常快的。 虽然 actor 在每个请求中都会构造，但 actor 状态键只是按需加载，而不是每次请求中都会加载。 然而，单独加载每个键会有一些开销。 这可以通过在状态中存储一个数据数组来缓解，用一些可用性换取速度。 不建议从一开始就这样做，而是在需要时作为一种优化手段。
