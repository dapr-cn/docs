...
[名称空间或组件作用域]({{< ref component-scopes.md >}}) 可用于限制对特定应用程序的组件访问。 添加到组件的这些应用程序作用域仅限制具有特定 ID 的应用程序才能使用该组件。
...
- `spec.metadata.publishingScopes`
  - 一个由分号分隔的应用程序列表和逗号分隔的主题列表，允许该应用程序发布到该主题列表
  - 如果在 `publishingScopes`（默认行为）中未指定任何内容，那么所有应用程序都可以发布到所有主题
  - 要拒绝应用程序发布信息到任何主题，请将主题列表留空 (`app1=;app2=topic2`)
  - 例如，`app1=topic1;app2=topic2,topic3;app3=`将允许app1发布到topic1，而不允许发布到其他任何主题，app2只能发布到topic2和topic3，而app3则无法发布任何信息。
- `spec.metadata.subscriptionScopes`
  - 一个由分号分隔的应用程序列表和逗号分隔的主题列表，允许该应用程序订阅该主题列表
  - 如果在 `subscriptionScopes`（默认行为）中未指定任何内容，那么所有应用程序都可以订阅所有主题
  - 例如，`app1=topic1;app2=topic2,topic3`将允许app1仅订阅topic1，而app2可以订阅topic2和topic3
- `spec.metadata.allowedTopics`
  - 一个逗号分隔的允许主题列表，对所有应用程序。
  - 如果未设置 `allowedTopics` （默认行为），则所有主题都有效。 如果存在，`subscriptionScopes`和`publishingScopes`仍然生效。
  - `publishingScopes` 或 `subscriptionScopes` 可以与 `allowedTopics` 结合使用，以添加细粒度的限制
- `spec.metadata.protectedTopics`
  - 一个逗号分隔的受保护主题列表，对所有应用程序。
  - 如果一个主题被标记为受保护，则必须通过 `publishingScopes` 或 `subscriptionScopes` 显式地授予应用程序发布或订阅权限，以便对其进行发布/订阅。
...
> 注意: 如果应用程序未列出 ( 例如， subscriptionScopes 中的 app1) ，那么允许订阅所有主题。 因为`allowedTopics`未使用，而app1没有任何订阅范围，因此它还可以使用上面未列出的其他主题。

## 示例 2: 限制允许的主题

当 Dapr 应用程序给主题发送信息时，主题将自动创建。 在某些情况下，这个主题的创建应该得到管理。 For example:
...
如果您的主题涉及敏感数据，则必须在 `publishingScopes` 和 `subscriptionScopes` 中明确列出每个新应用程序，以确保它无法读取或写入该主题。 或者，您可以将主题指定为'protected'（使用`protectedTopics`），并仅授予真正需要访问的特定应用程序访问权限。
