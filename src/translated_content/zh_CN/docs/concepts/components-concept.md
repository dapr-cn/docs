...
对于将数据返回到应用的任何组件，建议相应地设置 Dapr sidecar 的内存容量（进程或容器），以避免潜在的 OOM 恐慌。 例如，在docker中使用`--memory`选项。 对于 Kubernetes，请使用 `dapr.io/sidecar-memory-limit` 注解。 For processes this depends on the OS and/or process orchestration tools.\*
{{% /alert %}}
