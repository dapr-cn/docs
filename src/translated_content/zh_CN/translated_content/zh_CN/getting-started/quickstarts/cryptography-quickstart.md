...
让我们来看看Dapr的[cryptography构建块]({{< ref cryptography >}})。 在此快速入门中，您将创建一个应用程序，使用 Dapr 加密和解密数据的 API。 您将：
...
这个示例使用的是 Dapr SDK，它利用 gRPC 并且在使用加密 API 加密和解密消息时**强烈**推荐使用。
{{% /alert %}}
...
克隆[在Quickstarts存储库中提供的示例](https://github.com/dapr/quickstarts/tree/master/cryptography)。
...
早些时候，您在 `crypto-quickstarts` 中创建了一个名为 `keys` 的目录。 在[`local-storage`组件的YAML文件](https://github.com/dapr/quickstarts/tree/master/cryptography/components/local-storage.yaml)中，`path`元数据映射到新创建的`keys`目录。
...
[应用程序文件](https://github.com/dapr/quickstarts/blob/master/cryptography/javascript/sdk/crypto-quickstart/index.mjs) 使用您生成的RSA和AES密钥加密和解密消息和文件。 应用程序创建一个新的 Dapr SDK 客户端：
...
[应用程序文件](https://github.com/dapr/quickstarts/tree/master/cryptography/go/sdk/crypto-quickstart/app.go) 使用您生成的RSA和AES密钥加密和解密消息和文件。 应用程序创建一个新的 Dapr SDK 客户端：
...
请观看 [Dapr Community Call #83中的密码学API演示视频](https://youtu.be/PRWYX4lb2Sg?t=1148):
