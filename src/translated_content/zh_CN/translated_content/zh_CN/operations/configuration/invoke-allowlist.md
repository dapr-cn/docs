...
Access control enables the configuration of policies that restrict what operations _calling_ applications can perform, via service invocation, on the _called_ application. To limit access to a called applications from specific operations and HTTP verbs from the calling applications, you can define an access control policy specification in configuration.
