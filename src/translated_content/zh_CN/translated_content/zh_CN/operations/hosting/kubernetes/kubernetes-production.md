...
1. **启用 Dapr to App API 验证**。 这是你的应用程序和 Dapr sidecar 之间的通信。 To secure the Dapr API from unauthorized application access, [enable Dapr's token-based authentication]({{< ref "api-token.md" >}}).

1. **启用 Dapr to App API 验证**。 这是你的应用程序和 Dapr sidecar 之间的通信。 [Let Dapr know that it is communicating with an authorized application using token authentication]({{< ref "app-api-token.md" >}}).
