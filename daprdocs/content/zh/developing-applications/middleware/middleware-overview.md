...
Dapr uses [FastHTTP](https://github.com/valyala/fasthttp) to implement its HTTP server. 因此，您的 HTTP 中间件也需要编写为 FastHTTP handler。 Hence, your HTTP middleware needs to be written as a FastHTTP handler. 您的中间件需要实现 Middleware 接口，该接口定义 **GetHandler** 方法，该方法返回 **fasthttp.RequestHandler**:
