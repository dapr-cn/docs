...
启动后， Dapr sidecar 会构建中间件处理管道。 启动后， Dapr sidecar 会构建中间件处理管道。 其他中间件，由 Dapr [ configuration ]({{< ref configuration-concept.md >}}) 配置，按照定义的顺序添加到管道中。 管道适用于所有 Dapr API 端点，包括状态，发布/订阅，服务调用，绑定，安全和其他。
